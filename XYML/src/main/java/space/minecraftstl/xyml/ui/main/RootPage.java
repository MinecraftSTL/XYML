...
import space.minecraftstl.xyml.ui.nbt.NBTEditorPage;
import space.minecraftstl.xyml.ui.nbt.NBTFileType;
import space.minecraftstl.xyml.ui.versions.GameAdvancedListItem;
...
public class RootPage extends DecoratorAnimatedPage implements DecoratorPage {
    ...
    public RootPage() {
        EventBus.EVENT_BUS.channel(RefreshedVersionsEvent.class)
                .register(event -> onRefreshedVersions((XYMLGameRepository) event.getSource()));
    ...
    private void onRefreshedVersions(XYMLGameRepository repository) {
        runInFX(() -> {
            if (!checkedModpack) {
                checkedModpack = true;
