...
import space.minecraftstl.xyml.event.EventPriority;
...
import space.minecraftstl.xyml.ui.game.GameSettingsPage;
...
public class VersionPage extends DecoratorAnimatedPage implements DecoratorPage {
    ...
    private final ObjectProperty<XYMLGameRepository.InstanceReference> instanceReference = new SimpleObjectProperty<>();
    ...
    private void checkSelectedVersion() {
        runInFX(() -> {
            ...
            XYMLGameRepository repository = this.instanceReference.get().repository();
    ...
    public void setVersion(String version, XYMLGameRepository repository) {
        this.instanceReference.set(new XYMLGameRepository.InstanceReference(repository, version));
    }

    public void loadVersion(String version, XYMLGameRepository repository) {
    ...
    private void clearAssets() {
        ...
        XYMLGameRepository.InstanceReference currentInstanceReference = instanceReference.get();
    ...
    public XYMLGameRepository getRepository() {
        return Optional.ofNullable(instanceReference.get()).map(XYMLGameRepository.InstanceReference::repository).orElse(null);
    }

    public String getVersion() {
        return Optional.ofNullable(instanceReference.get()).map(XYMLGameRepository.InstanceReference::instanceId).orElse(null);
    }
    ...
    public interface GameInstanceLoadable {
        ...
        void loadInstance(XYMLGameRepository repository, @Nullable String instanceId);
    }
}
