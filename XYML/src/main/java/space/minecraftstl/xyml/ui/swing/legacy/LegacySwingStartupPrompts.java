...
/// Builds the Swing startup-prompt policy around the remaining legacy settings stores.
///
/// This adapter is deliberately isolated from the pure Swing prompt model while launcher settings still
/// expose their legacy observable-property representation.
@NotNullByDefault
public final class LegacySwingStartupPrompts {
    ...
    private LegacySwingStartupPrompts() {
    }

    /// Creates one idle prompt coordinator from the current initialized launcher state.
    ///
    /// This factory is invoked on the Swing event dispatch thread while legacy settings remain thread-bound.
    /// The returned coordinator performs later reads and writes through the serialized legacy-state bridge.
    ///
    ...
    public static StartupPromptCoordinator create(
        ...
        return new StartupPromptCoordinator(
                createEnvironment(),
                createStrings(),
                new LegacyStateGateway(),
                presenter,
                new LegacyEffects(closeApplication),
    ...
    /// Bridges startup prompt state reads and writes to legacy observable properties.
    @NotNullByDefault
    private static final class LegacyStateGateway implements StartupPromptStateGateway {
        /// Captures the current prompt state on the Swing event dispatch thread.
        @Override
        public StartupPromptSnapshot readSnapshot() {
            return callOnLegacyThread(() -> {
        ...
        public void acceptAgreement(int agreementVersion) {
            runOnLegacyThread(() -> SettingsManager.userState()
        ...
        public void restoreDefaultCacheDirectory() {
            runOnLegacyThread(() -> SettingsManager.settings()
        ...
        private static void putShownTip(String key, Object value) {
            ...
            runOnLegacyThread(() -> SettingsManager.state().getShownTips().put(key, value));
        }
    }

    /// Bridges process effects while keeping the coordinator worker independent from both UI threads.
    @NotNullByDefault
    private static final class LegacyEffects implements StartupPromptEffects {
        ...
        ///
        /// @param closeApplication unified application close command
        private LegacyEffects(Runnable closeApplication) {
    ...
    /// Runs one state mutation on the Swing-confined legacy state thread.
    ///
    /// @param operation mutation requiring legacy-state confinement
    private static void runOnLegacyThread(Runnable operation) {
        LegacyStateDispatcher.executeAndWait(Objects.requireNonNull(operation, "operation"));
    }

    /// Returns one non-null value produced on the Swing-confined legacy state thread.
    ///
    /// @param supplier value supplier requiring legacy-state confinement
    /// @param <T> non-null result type
    /// @return supplied value
    private static <T> T callOnLegacyThread(Supplier<T> supplier) {
        Objects.requireNonNull(supplier, "supplier");
        AtomicReference<@Nullable T> result = new AtomicReference<>();
        runOnLegacyThread(() -> result.set(Objects.requireNonNull(
                supplier.get(),
                "legacy state supplier returned null")));
        return Objects.requireNonNull(result.get(), "legacy state supplier did not run");
    }
}
