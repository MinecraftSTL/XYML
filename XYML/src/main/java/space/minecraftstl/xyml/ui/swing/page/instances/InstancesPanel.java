...
public final class InstancesPanel extends JPanel implements AutoCloseable {
    ...
    /// Creates an installed-instance panel on the EDT.
    ///
    /// @param model toolkit-neutral instance model and viewport source
    /// @param strings localized page text
    public InstancesPanel(InstancesModel model, InstancesStrings strings) {
        super(new MigLayout(
    ...
    private void configureComponents() {
        ...
        refreshButton.addActionListener(event -> model.refreshInstances());
        toolbar.add(refreshButton, "h 40!");

        addButton.setName("instancesAdd");
        addButton.setText(strings.addAction());
        addButton.addActionListener(event -> model.addInstance());
        ...
        manageButton.addActionListener(event -> model.manageSelectedInstance());
        statusBand.add(manageButton, "h 40!");
        add(statusBand, "growx");
    }
