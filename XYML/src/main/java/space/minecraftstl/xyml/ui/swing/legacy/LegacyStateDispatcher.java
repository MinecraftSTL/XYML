...
/// Centralizes thread-confined access to legacy observable state during the staged Swing migration.
///
...
@NotNullByDefault
public final class LegacyStateDispatcher {
    /// Prevents utility instantiation.
    private LegacyStateDispatcher() {
    }
    ...
    ///
    /// @param operation operation requiring legacy-state thread confinement
    ...
    public static void executeAndWait(Runnable operation) {
        ...
        try {
            ...
            throw new IllegalStateException("Interrupted while waiting for a legacy-state operation", failure);
            ...
            throw new IllegalStateException("Legacy-state operation failed", cause);
        }
    }

    /// Requires direct legacy property access to run on the Swing event dispatch thread.
    public static void requireEventThread() {
        if (!SwingUtilities.isEventDispatchThread()) {
            throw new IllegalStateException("Legacy state must be accessed on the Swing event dispatch thread");
        }
    }
}
