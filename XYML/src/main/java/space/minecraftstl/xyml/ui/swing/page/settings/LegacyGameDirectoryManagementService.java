...
/// Bridges process-wide legacy game-directory state to the immutable Swing management service contract.
///
...
/// rendering is therefore free of directory I/O and all legacy observable access remains confined to the EDT.
@NotNullByDefault
public final class LegacyGameDirectoryManagementService implements GameDirectoryManagementService {
    ...
    public LegacyGameDirectoryManagementService() {
    ...
    /// Releases legacy observable subscriptions.
    @Override
    public void close() {
        LegacyStateDispatcher.requireEventThread();
        if (closed) {
    ...
    /// Publishes a rebuilt snapshot after one legacy list or selection transition.
    ...
    /// Rejects calls after the adapter has released its legacy state subscriptions.
