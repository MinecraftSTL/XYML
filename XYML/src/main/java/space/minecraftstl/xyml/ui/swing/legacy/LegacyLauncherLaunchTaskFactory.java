...
/// Resolves one immutable Swing launch request against Swing-confined legacy stores.
///
...
/// resolving stable identifiers and constructing an unstarted task. Construction retains the legacy
...
@NotNullByDefault
public final class LegacyLauncherLaunchTaskFactory implements LaunchTaskFactory, AutoCloseable {
    /// Synchronous legacy-state bridge used only from a non-EDT preparation worker.
    private final Consumer<Runnable> legacyDispatcher;
    ...
    public LegacyLauncherLaunchTaskFactory(
        ...
        this.legacyDispatcher = LegacyStateDispatcher::executeAndWait;
    ...
    /// @param launchTaskBuilder legacy-thread task builder
    ...
    ///
    /// Legacy metadata maintenance may execute synchronously before this method returns.
    ///
    ...
    public Task<ManagedProcess> create(LaunchRequest request) {
        ...
        if (SwingUtilities.isEventDispatchThread()) {
            throw new IllegalStateException("Legacy launch task creation must not block the Swing EDT");
        }
