...
/// Owns one cancellable local launch-script export while preserving the legacy launch preparation chain.
///
/// Task construction runs on the caller-owned preparation executor because legacy metadata normalization may block.
...
@NotNullByDefault
public final class LegacyLaunchScriptExportService implements AutoCloseable {
    ...
    /// Creates the production service around the legacy stable-ID task factory.
    ///
    ...
    public LegacyLaunchScriptExportService(
            LegacyLauncherLaunchTaskFactory taskFactory,
    ...
    /// @param preparationExecutor caller-owned executor for synchronous task construction
    LegacyLaunchScriptExportService(
