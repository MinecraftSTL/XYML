...
import static space.minecraftstl.xyml.ui.FXUtils.ignoreEvent;
import static space.minecraftstl.xyml.ui.FXUtils.stringConverter;
import static space.minecraftstl.xyml.util.i18n.I18n.i18n;
import static space.minecraftstl.xyml.util.javafx.ExtendedProperties.selectedItemPropertyFor;
...
public class DownloadListPage extends Control implements DecoratorPage, VersionPage.GameInstanceLoadable {
    ...
    @Override
    public void loadInstance(XYMLGameRepository repository, @Nullable String instanceId) {
        this.instanceReference.set(new XYMLGameRepository.InstanceReference(repository, instanceId));
    ...
    private void search(String userGameVersion, RemoteAddonRepository.Category category, int pageOffset, String searchFilter, RemoteAddonRepository.SortType sort) {
        ...
        Task.supplyAsync(() -> {
            XYMLGameRepository.InstanceReference instanceReference = this.instanceReference.get();
    ...
    protected XYMLGameRepository.InstanceReference getInstanceReference() {
        if (versionSelection) {
            return new XYMLGameRepository.InstanceReference(instanceReference.get().repository(), selectedVersion.get());
        } else {
