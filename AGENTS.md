...
- “变更分支”指为任何开发改动创建的任务分支，包括新功能、修正、重构、文档、测试和构建变更；不使用“更新分支”，避免“更新”的读音和语义歧义。
- `dev` 是本仓库所有开发变更集成使用的直接集成分支。除用户在本轮明确指定其他基线或目标外，所有变更分支必须从当前 `dev` 尖端创建，禁止从 `main`、`alpha`、`beta`、稳定版发布分支或旧快照直接创建变更分支。
- 创建分支前必须确认真实基线：新分支的起点必须是当前 `dev` 尖端或其明确后代；若起点位于其他发布通道，必须先停止并改为从 `dev` 重新创建分支，不能带着错误历史继续开发。
- 变更分支完成并验证后，必须使用 `git merge --no-ff` 合并回当前 `dev`。除非用户在本轮明确要求，不得直接将变更分支合并到 `main`、`alpha`、`beta` 或远程分支，也不得隐式推送。
- 若发现已有变更分支不是从 `dev` 分出，必须先在合并前完成历史重写或重新基于 `dev` 提交；历史重写前保留可识别备份，重写后校验最终树与已验证结果一致。
