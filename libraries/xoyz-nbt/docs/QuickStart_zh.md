...
如果你初次使用 XoyzNBT，可以先阅读本节来快速了解 XoyzNBT 的基本用法。

## 读取 NBT 文件

XoyzNBT 支持读取和写入 NBT 文件（例如 `level.dat`、`playerdata/<UUID>.dat`）、
Anvil 文件和区域文件（`*.mca`/`*.mcr`）。

XoyzNBT 提供了 `NBTCodec` 类来读取和写入这些文件。
...
并通过 `NBTFile` 保留压缩格式、检测外部冲突和安全发布。完整用法与失败恢复规则参见[安全编辑指南](SafeEditing_zh.md)。
