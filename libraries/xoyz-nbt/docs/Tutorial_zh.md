...
如果你初次使用 XoyzNBT，可以先阅读 [快速入门](/docs/QuickStart_zh.md) 来快速了解 XoyzNBT 的基本用法。

## 基础

XoyzNBT 提供了对 NBT 树的基本抽象。
...
var compoundTag = new CompoundTag()
        ...
        .addInt("int", 123)
        // 添加一个 StringTag 子标签，其名称为 "str"，值为 "XoyzNBT"
        .addString("str", "XoyzNBT")
        // addTag 方法可以添加任意子标签
...
XoyzNBT 支持读取和写入 NBT 文件、Anvil 文件（`.mca`）和区域文件（`.mcr`）。
...
> [!NOTE]
> 
> XoyzNBT 需要运行时存在 [lz4-java](https://github.com/lz4/lz4-java) 库才能读取使用 LZ4 压缩的 NBT 数据。
> 
...
每个 Anvil 文件和 Region 文件中都存储了一个**区域（Region）**，在 XoyzNBT 中使用 `ChunkRegion` 类来表示。

每个区域都包含了 32 x 32 个区块，XoyzNBT 使用 `Chunk` 类来表示一个区块。

关于区域和更多信息可以在 Minecraft Wiki 中查看：[区域文件格式](https://zh.minecraft.wiki/w/%E5%8C%BA%E5%9F%9F%E6%96%87%E4%BB%B6%E6%A0%BC%E5%BC%8F)。

在 XoyzNBT 中，可以使用 `NBTCodec` 的 `readRegion` 方法来读取 Anvil 文件和 Region 文件：
...
由于区域文件格式的限制，无法实现流式写入，所以以 `OutputStream` 为目标时 XoyzNBT 会先在内存中构建完整的区域文件，
再写入到输出流中，这可能会导致内存占用较大。
我们更建议使用 `SeekableByteChannel` 作为目标，这样 XoyzNBT 可以逐个区块写入通道，避免额外的内存占用。
...
XoyzNBT 提供了 `ExternalChunkAccessor` 接口来读取和写入外部区块文件。
...
XoyzNBT 还支持读取和写入 SNBT（Stringified Named Binary Tag）数据。
...
```java
Tag _ = SNBTCodec.of().readTag("XoyzNBT");
