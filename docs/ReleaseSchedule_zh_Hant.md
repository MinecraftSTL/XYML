...
| 渠道 | GitHub 下載 | 其他下載位置 | 回饋入口 |
| --- | --- | --- | --- |
| 穩定版 | 公開 | 發佈 | 公開 |
| 公測版 | 公開 | 可發佈 | 公開 |
| 內測版 | 公開預發佈 | 不發佈 | 受限測試計畫 |
| 開發版 | 公開預發佈 | 不發佈 | 受限測試計畫 |

內測版和開發版會在 GitHub Releases 中公開提供下載。二進位檔案可以公開取得，並不代表回饋入口也公開：這兩個渠道只透過受限測試計畫收集回饋，公開 Bug 表單只接受目前穩定版和公測版的問題。
...
正式建置會拒絕缺失或格式錯誤的發佈輸入。`main`、`beta`、`alpha` 和 `dev` 分支的建置會保留解析後的渠道版本；其他所有分支的建置都會附加一個空段。例如，使用預設開發版渠道的本機功能分支建置為 `1.0.0.0.0.0.`。

GitHub 發佈工作流程必須從與所選渠道一致的分支執行。它會建立公開 Release，把公測版、內測版和開發版標記為預發佈，並更新對應渠道的升級描述檔。只有穩定版和公測版建置會交給 GitHub 以外的分發工作。
