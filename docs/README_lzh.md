<!-- #BEGIN COPY -->
<!-- #PROPERTY NAME=TITLE -->
<div align="center">
    <img src="/XYML/src/main/resources/assets/img/icon@8x.png" alt="XYML Logo" width="64"/>
</div>

<h1 align="center">xOyz Minecraft Launcher</h1>
<!-- #END COPY -->

<!-- #BEGIN COPY -->
<!-- #PROPERTY NAME=BADGES -->
<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-repo-blue?style=flat-square&logo=github)](https://github.com/MinecraftSTL/XYML)


</div>
<!-- #END COPY -->

---

<!-- #BEGIN LANGUAGE_SWITCHER -->
English ([Standard](README.md), [uʍoᗡ ǝpᴉsd∩](README_en_Qabs.md)) | **中文** ([简体](README_zh.md), [繁體](README_zh_Hant.md), **文言**) | [日本語](README_ja.md) | [español](README_es.md) | [русский](README_ru.md) | [українська](README_uk.md)
<!-- #END LANGUAGE_SWITCHER -->

### 概敘

XYML 者，開源之礦藝啟器也。善理改囊，可自定遊戲，自動裝設 Forge、NeoForge、Cleanroom、Fabric、Legacy Fabric、Quilt、LiteLoader、OptiFine 諸改囊，亦能制改囊集，界面隨心易之。

XYML 跨域廣矣。Windows、Linux、macOS、FreeBSD 諸常見械綱，x86、ARM、RISC-V、MIPS、LoongArch 諸算器架構，皆可運行。
憑此器，可遊《礦藝》於諸機之間，無礙也。  

欲知 XYML 所支平臺，詳觀[此表](PLATFORM_zh_Hant.md)。

## 下載

可從下述諸途得 XYML：

- [XYML 官方網站](https://github.com/MinecraftSTL/XYML/releases)
- [GitHub Release](https://github.com/MinecraftSTL/XYML/releases)
- [CNB Release](https://github.com/MinecraftSTL/XYML/releases)

## 與貢

XYML 乃社群共驅之開源項目，迎諸君獻碼或建言。

參與之道如下：

- 於 GitHub 上[創建 Issue](https://github.com/MinecraftSTL/XYML/issues/new/choose)，以報謬誤或求功能。
- 於 GitHub 上 Fork 倉庫，並[提交 Pull Request](https://github.com/MinecraftSTL/XYML/compare)，以貢代碼。

貢獻之前，請閱[貢獻指南](./Contributing_zh_Hant.md)，中含：

- [如何自源碼構建並執行 XYML](./Contributing_zh_Hant.md#構建-xyml)
- [以勘誤選項調整 XYML 之行為](./Contributing_zh_Hant.md#除錯選項)

## 貢獻者

自乙未年（2015）始，百二十余人參與 XYML，謝其勤勞！

[![Contributors](https://contrib.rocks/image?repo=MinecraftSTL/XYML)](https://github.com/MinecraftSTL/XYML/graphs/contributors)

## 開源之約

詳見 [README_zh_Hant.md](README_zh_Hant.md#開源協議)。
